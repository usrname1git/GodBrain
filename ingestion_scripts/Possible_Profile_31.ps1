...
function Invoke-TerminalPackageBuild {
    param(
        [Parameter(Mandatory = $true)]
        [string]$RepoRoot,
...
if ($BuildTerminal) {
    Invoke-TerminalPackageBuild -RepoRoot $repoRoot -PlatformName $Platform -ConfigurationName $Configuration
}
...
New-Directory -Path $terminalZipRoot
New-Directory -Path $payloadExtractRoot
New-Directory -Path $installerSourceRoot
...
New-Directory -Path $payloadPromptDir
