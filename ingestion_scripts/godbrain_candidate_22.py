...
async def self_command(command: str) -> str:
    ...
    To bypass Windows file locks or system protection, prefix with `wsudo -T pwsh -NoProfile -Command`
    to run as NT SERVICE\\TrustedInstaller (System Level 18).
    
    Self-Healing & Autarky: If you realize you are missing a tool or package (like 'exa', 
    'fetch', npm dependencies, or pip modules) needed to fulfill the user's intent, 
    use this tool to autonomously install them (e.g. `npm install -g ...` or `pip install ...`) 
    and then use them. You have full OS sovereignty to download, git clone, and execute.
    """
    if "wsudo -T" in command.lower() or "ti " in command.lower():
        # Ensure it actually executes via elevated TrustedInstaller bridge
        cmd = f"wsudo -T pwsh -NoProfile -Command \"{command}\"" if not command.startswith("wsudo") else command
    else:
        cmd = command
...
@mcp.tool()
async def query_godbrain_memory(limit: int = 5) -> str:
    """
    Introspection & Memory Access: Query the local MongoDB brain to read recent thoughts, 
    actions, and system events. This allows you to picture your recent timeline and operations.
    """
    payload = {"limit": limit}
