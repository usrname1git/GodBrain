...
async def run_synthesizer_loop():
    ...
    for filename in files_to_process:
        ...
        # If the file is massive, we chop it into chunks
        ...
        if len(chunks) > 1:
            print(f"     [!] Massive file detected ({len(raw_text)} chars). Chunking into {len(chunks)} pieces.")
            
        for idx, chunk in enumerate(chunks):
            if len(chunks) > 1:
                print(f"     -> Processing Chunk {idx + 1}/{len(chunks)}...")
                
            synthesis = await synthesize(chunk)
            
            # 4. Save Final Distilled Knowledge 
            await engine.save_thought(
                content=f"[SYNTHESIS: {filename} (Part {idx + 1}/{len(chunks)})]\n{synthesis}",
