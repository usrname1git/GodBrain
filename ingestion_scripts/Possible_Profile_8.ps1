...
# Classic conhost re-links BufferWidth to WindowWidth on every resize, so the
# "-1" gap that PSReadLine needs gets snapped shut and PSReadLine crashes (taking
# half the host with it). conhost can't keep Buffer > Window reliably, so under
# ConsoleHost we simply unload PSReadLine — the only fix immune to that relink.
# pwsh-in-Intelligent-Terminal (ConPTY) and the VS Code host keep PSReadLine.
...
if ($Host.Name -eq 'Visual Studio Code Host') {
    try {
        $Raw = $Host.UI.RawUI
        $NewSize = $Raw.BufferSize
        $NewSize.Width = $Raw.WindowSize.Width - 1 # Subtract 1 to prevent PSReadLine crash
        $NewSize.Height = 9000 # Keep long history
        $Raw.BufferSize = $NewSize
    } catch {}
}
