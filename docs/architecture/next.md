...
The next phase is **this host's closed loop**, not a new architecture. Strengthen
the check that already exists. Do not hire Agent Factory to look busy.

Index: [`ARCHITECTURE.md`](../../ARCHITECTURE.md). What already ships:
[`current.md`](current.md). Later / not-this-phase: [`future.md`](future.md).

B-line: Windows OS/network on this machine — detect → reason → allowlist patch
→ verify. Not tanks, not AppX eviction, not fleet DISM.
...
llama-server Gemma 12B Q4, MTP on by default, `-NoDraft` if it misbehaves.
