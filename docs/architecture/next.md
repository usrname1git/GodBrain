...
The b-line closed-loop phase **shipped**. This file is no longer a backlog.

Live desk: [`current.md`](current.md). Snapshot of what this page used to
be: [`b-line.md`](b-line.md). Later / not-this-host: [`future.md`](future.md).
Index: [`ARCHITECTURE.md`](../../ARCHITECTURE.md).

What “done” meant, and where it lives now:

- Heal/Watch restore `:27017` / `:8084` / `:8000` / `:8083`; desk tests
  fail-closed; Heal never kills or cocktails — [`current.md`](current.md)
  Heal / Watch / SRE.
- Inbox → Librarian candidates; `/pending` is a judge list; playbooks stay
  candidate — Alexandria in [`current.md`](current.md).
